...
Цель проекта шире — воспроизводимое LPDE вместе с зависимостями, конфигурацией и сессией, — но текущий `install.sh` устанавливает только QML и `qmldir`. Он не устанавливает пакеты и не разворачивает примерный профиль из `extra/hypr/`. Границы и текущий этап описаны в [документации](docs/README.md).
...
Запускайте установщик без `sudo`. Он копирует все найденные в репозитории QML-файлы и `qmldir` в `~/.config/quickshell/`, предварительно перемещая существующую конфигурацию в `~/.config/quickshell.backup.*`, и сообщает об отсутствии некоторых команд. Зависимости нужно установить отдельно. Скрипт не проверяет версии, QML-импорты и шрифт, не запускает оболочку и не меняет настройки Hyprland.

Повторная установка создаёт новую резервную копию. При запуске из самого каталога установленной конфигурации скрипт ничего не меняет. Используйте путь `~/.config/quickshell/`: некоторые службы обращаются к нему напрямую. Сохранённый `wallpaper.conf` остаётся в backup и сейчас автоматически не переносится в новый каталог.
