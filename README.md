# Оболочка Quickshell для LPDE

Текущая поставка проекта — модульная оболочка на [Quickshell](https://quickshell.outfoxxed.me/) для подготовленной сессии Hyprland: панель, меню приложений, уведомления, индикаторы громкости и яркости, управление обоями и мониторами, а также режим временного запрета бездействия.

Цель проекта шире — воспроизводимое LPDE вместе с зависимостями, конфигурацией и сессией, — но текущий `install.sh` устанавливает только QML, `qmldir` и файлы из `configs/`; он не устанавливает пакеты и не запускает/применяет профиль автоматически. Границы и текущий этап описаны в [документации](docs/README.md).

Модули можно использовать вместе или подключать по отдельности. Все они используют общую тему `config/Theme.qml` и настройки `config/Config.qml`, импортируемые в QML как `qs.config`. Вопросы и предложения можно оставлять в Issues репозитория.
...
| `IdleInhibitor` | Временный запрос Wayland idle-inhibit для настроенной политики бездействия |
...
Запускайте установщик без `sudo`. Он копирует все найденные в репозитории QML-файлы и `qmldir` в `~/.config/quickshell/`, а содержимое `configs/` — в соответствующие пути под `~/.config/`, предварительно перемещая существующие управляемые пути в backup. Остальной `~/.config` не заменяется. Зависимости нужно установить отдельно. Скрипт не проверяет версии, QML-импорты и шрифт, не запускает оболочку и не применяет/reload-ит настройки Hyprland.

Повторная установка создаёт новую резервную копию управляемых путей. При запуске из самого каталога установленной конфигурации скрипт ничего не меняет. Используйте путь `~/.config/quickshell/`: некоторые службы обращаются к нему напрямую. Сохранённый `wallpaper.conf` остаётся в backup и автоматически не переносится в новый каталог.
